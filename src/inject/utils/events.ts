...
export const initBaseEvents = () => {
  window.addEventListener(MOUSE_UP, e =>
    EventBus.emit(EVENTS_ENUM.MOUSE_UP_BUBBLE, e),
  );
  window.addEventListener(MOUSE_DOWN, e =>
    EventBus.emit(EVENTS_ENUM.MOUSE_DOWN_BUBBLE, e),
  );
  window.addEventListener(
    MOUSE_UP,
    e => EventBus.emit(EVENTS_ENUM.MOUSE_UP_CAPTURE, e),
    true,
  );
  window.addEventListener(
    MOUSE_DOWN,
    e => EventBus.emit(EVENTS_ENUM.MOUSE_DOWN_CAPTURE, e),
    true,
  );
  window.addEventListener(
    COPY,
    e => EventBus.emit(EVENTS_ENUM.COPY_CAPTURE, e),
    true,
  );
  window.addEventListener(
    KEY_DOWN,
    e => EventBus.emit(EVENTS_ENUM.KEY_BOARD_CAPTURE, e),
    true,
  );
  window.addEventListener(
    CONTEXT_MENU,
  ...
  window.addEventListener(
    DOM_READY,
    e => EventBus.emit(EVENTS_ENUM.DOM_LOADED, e),
    true,
  );
  window.addEventListener(
    PAGE_LOADED,
    e => EventBus.emit(EVENTS_ENUM.PAGE_LOADED, e),
    true,
  );
};
